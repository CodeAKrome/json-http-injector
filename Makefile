...
lint:
	...
	flake8 --max-line-length 120 --per-file-ignores=src/micro-gp.py:F401 --per-file-ignores=src/lib/spacelab.py:F401 src/*.py src/lib/spacelab.py
...
build:
	docker build -t micro-gp:latest .
deploy:
	docker compose up
